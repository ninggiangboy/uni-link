...
public class CreatePasswordResetUseCase implements UseCaseService {

    private final AccountRepository accountRepository;
    private final AccountOtpDeliveryService accountOtpDeliveryService;

    public CreatePasswordResetResponse execute(CreatePasswordResetRequest request) {
        ...
        if (accountOpt.isEmpty() || !accountOpt.get().isActive()) {
            ...
            return new CreatePasswordResetResponse(UUID.randomUUID().toString());
        }

        Account account = accountOpt.get();

        // ResetPassword will load latest active OTP with this purpose.
        AccountOtp otp = accountOtpDeliveryService.sendEmailOtp(account.getId(), request.email(), OtpPurpose.PASSWORD_RESET);
        return new CreatePasswordResetResponse(otp.getUuid());
    }
}
