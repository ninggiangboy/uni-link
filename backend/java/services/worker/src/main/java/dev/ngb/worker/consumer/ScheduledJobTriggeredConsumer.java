...
import dev.ngb.infrastructure.kafka.scheduled_job.ScheduledJobTriggeredEvent;
...
@Slf4j
public class ScheduledJobTriggeredConsumer {

    private final ObjectMapper objectMapper;
    private final AttachmentJobHandlers attachmentJobHandlers;

    @KafkaListener(topics = TopicNames.SCHEDULED_JOB_TRIGGERED)
    public void consume(String message) {
        ScheduledJobTriggeredEvent scheduledJob = parse(message);
    ...
    private ScheduledJobTriggeredEvent parse(String message) {
        try {
            EventPublicationEntity publication = objectMapper.readValue(message, EventPublicationEntity.class);
            return objectMapper.readValue(publication.getPayload(), ScheduledJobTriggeredEvent.class);
